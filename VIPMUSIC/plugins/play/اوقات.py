...
s = Session()
@app.on_message(filters.regex(r"^(مواقيت صلاة|مواقيت صلاه|صلوات|اوقات)"))
async def sendAdhan(_: Client, message: Message) -> None:
    address: str = message.text.rsplit(maxsplit=1)[-1]
    if address in ["مواقيت صلاة", "مواقيت صلاه", "صلوات","اوقات"]: return await message.reply("- اكتب اسم المنطقه بجانب الأمر،")
